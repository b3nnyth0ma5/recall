...
/**
 * Upload a document to the database and trigger embedding generation
 * Note: This is a simplified version that stores document metadata only.
 * In production, you would upload the actual document file to storage.
 */
export async function uploadDocumentToDatabase(
  ...
  try {
    console.log('[uploadDocumentToDatabase] Uploading document:', documentName);
    console.log('[uploadDocumentToDatabase] File size:', fileSizeMB, 'MB');
    console.log('[uploadDocumentToDatabase] Recall ID:', recallId);
    console.log('[uploadDocumentToDatabase] Document URI:', documentUri);

    const { data: { user } } = await supabase.auth.getUser();
    if (!user) {
      console.error('[uploadDocumentToDatabase] No authenticated user');
      return null;
    }

    // Insert document record
    // Note: In production, you would first upload the document file to storage
    // and store the storage URL. For now, we're just storing metadata.
    const { data: documentData, error: insertError } = await supabase
      ...
      .insert({
        ...
        // preview_cdn_url will be generated by the backend in production
      })
      .select('id')
      .single();

    if (insertError || !documentData) {
      console.error('[uploadDocumentToDatabase] Error inserting document:', insertError);
      return null;
    }

    console.log('[uploadDocumentToDatabase] Document inserted with ID:', documentData.id);
    console.log('[uploadDocumentToDatabase] Document embedding will be generated automatically by database trigger');
...
export async function deleteDocumentRecord(documentId: string): Promise<boolean> {
  try {
    ...
    if (error) {
      console.error('[deleteDocumentRecord] Error deleting document:', error);
      return false;
    }

    console.log('[deleteDocumentRecord] Document deleted successfully');
    return true;
  } catch (error) {
