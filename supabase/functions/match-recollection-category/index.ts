 ...
 */ Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: imagesData, error: imagesError } = await supabase.from('recall_images').select('id, ocr_text, image_explanation').eq('recall_id', recallId);
    if (imagesError) {
    ...
    const { data: categoriesData, error: categoriesError } = await supabase.from('recollection_categories').select('id, category_name');
    ...
    console.log(`Found ${matchingCategories.length} matching categories (score >= 75):`, matchingCategories.map((m)=>`${m.categoryName} (${m.score})`));
    ...
    if (matchingCategories.length > 0) {
      ...
      const { error: deleteError } = await supabase.from('recollections').delete().eq('recall_id', recallId).eq('user_id', recallData.user_id);
      if (deleteError) {
      ...
      const { error: insertError } = await supabase.from('recollections').insert(recollectionsToInsert);
      if (insertError) {
