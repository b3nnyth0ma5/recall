 ...
 */ async function extractLocationEntities(query, openaiApiKey) {
  try {
...
If no location intent is detected, return hasLocationIntent: false.`;
    ...
    return {
      ...
      proximity: nerResult.proximity || 5,
  ...
  try {
    ...
    if (searchRecallsResponse.error) {
      ...
      return new Response(JSON.stringify({
        ...
        details: searchRecallsResponse.error
      }), {
        status: 500,
        headers: {
          'Content-Type': 'application/json',
    ...
    const filteredResults = filterByLocation(searchResults, {
